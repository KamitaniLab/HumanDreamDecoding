% function convertRawDatatoPreprocessedDataCell
%
% [note]
...
%% initial settings
% cd('../science2013')
cd('/home/mu/horikawa-t/toolbox/public/science2013/')
% add path to functions
addpath('./func/');
% location of the donwloaded data
p.datdir = './data/';
% location of the save directory
p.savdir = setdir('./preproc_data_cell/');
% # of subject
nSubjects = 3;

%% Read & preprocess fmri data: create decoding sample (it takes about 30 minutes)
for i = 1:nSubjects
    p.subjectID=i;

    fprintf('Read data for Subject%d======\n',i);tic
    [Ds,Dp,props] = readfMRIData(p);toc
    % if you want whole brain data, save Ds and Dp
    
    fprintf('Preprocess data======\n');tic
    [pDs,pDp,props] = preprocfMRIData_test_cell(Ds,Dp,props);toc
